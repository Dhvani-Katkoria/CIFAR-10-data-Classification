...
for img in images :
    ...
    if (descriptor is not None):
        ...
        ret, label, centers = cv2.kmeans(vectorized, 100, None, criteria, 10, cv2.KMEANS_PP_CENTERS)#clusters and iterations
        freq = np.histogram(label,bins=range(0,101))[0].tolist()
        features+=[freq]
    else:
        index+=[id]
    id+=1
...
features+=[labels]
with open('cifar_sift_train.csv', 'w', newline='') as file:
...
with open('cifar_sift_test.csv', 'w', newline='') as file:
    writer = csv.writer(file, delimiter=',')
    writer.writerows(features)
